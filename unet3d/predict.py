...
def volumetric_predictions(is_training, is_dsv, is_half, model_filename, filenames, prediction_dir, model_name, n_features, window,
    ...
    if package == "pytorch":
        pytorch_volumetric_predictions(is_training=is_training,
                                       is_dsv=is_dsv,
                                       is_half=is_half,
...
def load_volumetric_model(is_training, is_dsv, is_half, model_name, model_filename, n_outputs, n_features, n_gpus, strict, **kwargs):
    from unet3d.models.pytorch.build import build_or_load_model
    model = build_or_load_model(model_name=model_name, model_filename=model_filename, n_outputs=n_outputs, is_training=is_training, is_dsv=is_dsv, is_half=is_half,
...
def load_volumetric_model_and_dataset(is_training, is_dsv, is_half, model_name, model_filename, model_kwargs, n_outputs, n_features,
                                      strict_model_loading, n_gpus, sequence, sequence_kwargs, filenames, window,
                                      spacing, metric_names):
    if model_kwargs is None:
        model_kwargs = dict()

    model = load_volumetric_model(is_training=is_training, is_dsv=is_dsv, is_half=is_half, model_name=model_name, model_filename=model_filename, n_outputs=n_outputs,
...
def pytorch_predict_batch_array(model, batch, n_gpus=1):
    ...
    for batch_idx in range(len(batch)):
        x1 = batch[batch_idx][:, :, :, 0]
        x2 = batch[batch_idx][:, :, :, 1]
        x3 = batch[batch_idx][:, :, :, 2]
        x4 = batch[batch_idx][:, :, :, 3]
        ...
        batch[batch_idx] = np.concatenate([batch[batch_idx], x_equalize_hist], axis=3)
...
def pytorch_volumetric_predictions(is_training, is_dsv, is_half, model_filename, model_name, n_features, filenames, window,
    ...
    model, dataset, basename = load_volumetric_model_and_dataset(is_training, is_dsv, is_half, model_name, model_filename, model_kwargs, n_outputs,
    ...
    with torch.no_grad():
        ...
        for idx in range(len(dataset)):
            ...
            print(x_image.shape, ref_image.shape)
