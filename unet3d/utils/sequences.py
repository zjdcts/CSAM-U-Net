...
def normalize_image_with_function(image, function, volume_indices=None, **kwargs):
    data = get_nibabel_data(image)  # nibabel_image.get_fdata()
...
class BaseSequence(Sequence):
    def __init__(self, filenames, batch_size, target_labels, window, spacing, classification='binary', shuffle=True,
        ...
        self.filenames = filenames  # len(filenames):369  config['training_filenames']
        self.inputs_per_epoch = inputs_per_epoch  # None
        ...
        self.iterations_per_epoch = iterations_per_epoch  # iterations_per_epoch:1
...
class HCPRegressionSequence(BaseSequence, HCPParent):
    def __init__(self, filenames, batch_size, window, spacing, metric_names, classification=None,
        ...
        # .normalize.zero_mean_normalize_image_data
    ...
    def normalize_image(self, image):
        # print(image.get_data().shape) (240, 240, 155, 4)
        if self.normalize:
...
class WholeVolumeAutoEncoderSequence(WholeVolumeToSurfaceSequence):
    ...
    def resample_image(self, input_filenames):  # input_filenames: item
        feature_image = self.format_feature_image(input_filenames=input_filenames)
        # print(feature_image.shape) (80, 96, 64, 4)
        # print(self.target_index) 2
        target_image = self.resample_target(self.load_target_image(feature_image, input_filenames),
                                            feature_image)
        # print(target_image.shape) (80, 96, 64, 1)
        # print(type(target_image)) <class 'nibabel.nifti1.Nifti1Image'>
        feature_image = augment_image(feature_image,
                                      additive_noise_std=self.additive_noise_std,
                                      additive_noise_probability=self.additive_noise_probability,
                                      augment_blur_mean=self.augment_blur_mean,
    ...
    def load_feature_image(self, input_filenames):  # input_filenames: item
        if self.extract_sub_volumes:  # False
    ...
    def format_feature_image(self, input_filenames, return_unmodified=False):  # input_filenames: item
        unmodified_image = self.load_feature_image(input_filenames)  # 4 模态 nifti 图像
...
class WholeVolumeSegmentationSequence(WholeVolumeAutoEncoderSequence):
    ...
    def resample_input(self, input_filenames):
        input_image, target_image = self.resample_image(input_filenames)  # input_filenames: item
