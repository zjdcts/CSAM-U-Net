...
class MirroredDecoder(nn.Module):
    ...
    def calculate_layer_widths(self, depth):
        # print(depth)
        # print(self.layer_widths[depth])
        if self.layer_widths is not None:
            if depth > 0:
                out_width = self.layer_widths[depth - 1]
            else:
                out_width = self.layer_widths[0]
            if depth == len(self.layer_blocks) - 1:
                in_width = self.layer_widths[depth]
            else:
                in_width = self.layer_widths[depth] * 2
        else:
            if depth > 0:
